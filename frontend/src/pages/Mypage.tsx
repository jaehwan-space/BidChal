...
export function Mypage() {
  ...
  const handleGift = async (e: React.FormEvent) => {
    e.preventDefault();
    const email = giftTarget.trim();
    const amount = Number(giftAmount);

    // 클라이언트 사전 검증
    if (!email) { toast.error('받는 사람의 이메일을 입력해주세요.'); return; }
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) { toast.error('올바른 이메일 형식이 아닙니다.'); return; }
    if (!giftAmount || isNaN(amount) || amount <= 0) { toast.error('1P 이상의 올바른 금액을 입력해주세요.'); return; }
    if (!Number.isInteger(amount)) { toast.error('정수 단위로만 선물할 수 있습니다.'); return; }
    ...
    try {
      ...
      if (!res.ok) throw new Error(resData.error || '포인트 선물에 실패했습니다.');
      toast.success(`${email}님에게 ${amount.toLocaleString()}P를 선물했습니다.`);
      setActiveModal('none');
      setGiftTarget(''); setGiftAmount('');
      fetchMypage();
    } catch (error: any) { toast.error(error.message || '알 수 없는 오류가 발생했습니다.'); }
  };
  ...
  const submitCoupon = async (code: string) => {
    ...
    if (trimmed.length < 4) {
      toast.error('쿠폰 번호가 너무 짧습니다. 올바른 쿠폰 코드를 입력해주세요.');
      return;
    }
    try {
